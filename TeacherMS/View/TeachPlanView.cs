...
{
    public partial class TeachPlanView : UserControl
    {
        public TeachPlanView()
        {
            ...
            Load += (s, e) =>
            {
                dataGridView.DataSource = new TeachPlanService().Select();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            int result = new TeachPlanService().Insert(model);
            if (result > 0)
            {
        ...
        private void buttonEdit_Click(object sender, EventArgs e)
        {
            ...
            if (rows.Count > 0)
            {
                ...
                var list = dataGridView.DataSource as List<TeachPlan>;
                var model = list[index];
                var Form = new EditTeachPlanForm(model);
        ...
        private void buttonDelete_Click(object sender, EventArgs e)
        {
            ...
            if (rows.Count > 0)
            {
                ...
                var list = dataGridView.DataSource as List<TeachPlan>;
                var model = list[index];
                var count = new TeachPlanService().Delete(model);
                if (count > 0)
                {
